...
export class EditCoordinatePlaneTool extends Tool {
  protected override _cursor: Cursor = Cursor.EDIT_NODE;
  private _editableElement: CoordinatePlaneView | null = null;
  ...
  public get editableElement(): CoordinatePlaneView | null {
    return this._editableElement;
  }
  public set editableElement(editableElement: CoordinatePlaneView | null) {
    if (!editableElement) {
      return;
    }
  ...
  public override on(call: boolean = true): boolean {
    ...
    this.focus.children.forEach((child: ElementView) => {
      if (child instanceof CoordinatePlaneView) {
